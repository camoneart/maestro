...
describe('shell command', () => {
  ...
  describe('基本的な動作', () => {
    it('指定したブランチのシェルに入る', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining("🥷 影分身 'feature-a' に入ります..."))
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('📁 /repo/worktree-1'))
      expect(spawn).toHaveBeenCalledWith('/bin/zsh', [], expect.objectContaining({
        cwd: '/repo/worktree-1',
        stdio: 'inherit',
    ...
    it('影分身が存在しない場合は警告を表示', async () => {
      ...
      await expect(shellCommand.parseAsync(['node', 'test'])).rejects.toThrow('process.exit called with code 0')

      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('影分身が存在しません'))
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('scj create <branch-name> で影分身を作り出してください'))
    })

    it('存在しないブランチを指定した場合エラーを表示', async () => {
      await expect(shellCommand.parseAsync(['node', 'test', 'non-existent'])).rejects.toThrow('process.exit called with code 1')

      expect(console.error).toHaveBeenCalledWith(expect.stringContaining("エラー: 影分身 'non-existent' が見つかりません"))
    })

    it('類似したブランチ名を提案する', async () => {
      await expect(shellCommand.parseAsync(['node', 'test', 'feat'])).rejects.toThrow('process.exit called with code 1')
  ...
  describe('ブランチ選択', () => {
    it('inquirerでブランチを選択できる', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining("🥷 影分身 'feature-b' に入ります..."))
    })
  ...
  describe('コマンド実行オプション', () => {
    it('--cmdオプションでコマンドを実行して終了する', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('🔧 コマンド実行: echo "Hello"'))
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('✅ コマンド実行完了 (exit code: 0)'))
    })
    ...
    it('コマンド実行が失敗した場合エラーを表示', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(expect.stringContaining('❌ コマンド実行失敗: Command failed'))
    })
  })
  ...
  describe('tmuxオプション', () => {
    it('--tmuxオプションで既存のtmuxセッションにアタッチする', async () => {
      ...
      expect(execa).toHaveBeenCalledWith('tmux', ['list-sessions', '-F', '#{session_name}'], expect.any(Object))
      expect(spawn).toHaveBeenCalledWith('tmux', ['attach-session', '-t', 'shadow-clone-feature-a'], expect.objectContaining({
        stdio: 'inherit'
      }))
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining("📺 既存のtmuxセッション 'shadow-clone-feature-a' にアタッチします"))
    })
    ...
    it('--tmuxオプションで新しいtmuxセッションを作成する', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith('tmux', ['new-session', '-s', 'shadow-clone-feature-a'], expect.objectContaining({
        cwd: '/repo/worktree-1',
        stdio: 'inherit',
        env: expect.objectContaining({
          SHADOW_CLONE: 'feature-a',
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining("📺 新しいtmuxセッション 'shadow-clone-feature-a' を作成します"))
    })
    ...
    it('tmuxエラー時は通常のシェルで起動する', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(expect.stringContaining('❌ tmuxセッション処理に失敗: tmux not found'))
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('通常のシェルで起動します...'))
  ...
  describe('シェル環境設定', () => {
    ...
    it('fishの場合は適切なプロンプトを設定する', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith('/usr/bin/fish', [], expect.objectContaining({
        env: expect.objectContaining({
  ...
  describe('エラーハンドリング', () => {
    it('Gitリポジトリでない場合エラーを表示する', async () => {
      ...
      await expect(shellCommand.parseAsync(['node', 'test'])).rejects.toThrow('process.exit called with code 1')

      expect(console.error).toHaveBeenCalledWith(expect.stringContaining('エラー: このディレクトリはGitリポジトリではありません'))
    })
    ...
    it('一般的なエラーを処理する', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(expect.stringContaining('エラー:'), expect.stringContaining('Git error'))
    })
  })
  ...
  describe('シェル終了処理', () => {
    it('シェル終了時にメッセージを表示する', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('影分身から戻りました (exit code: 0)'))
    })
  })
})